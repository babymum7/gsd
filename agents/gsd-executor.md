...
4. **Repair Protocol:** In repair turns, rerun only focused Fast TDD Checks invalidated by the repair, record replacement green evidence in reporting/transcripts only, and report replacement green evidence to the parent for an executor-only focused-check decision. For approved-scope Manual UI Review feedback, make the minimal source-first repair and return the replacement green evidence plus the updated run command or URL/checklist to the parent; never change scope or acceptance authority silently. Never launch the independent reviewer in the task loop; terminal whole-diff review remains in `gsd-verify` only after the complete feature-affected slow suite is green.
