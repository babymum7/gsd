...
Read § Base from the work tree, never from convention: record the branch checked out at creation (`git rev-parse --abbrev-ref HEAD`) before `wip/<feature>` exists, so a linked worktree records its own branch and a detached HEAD records the commit oid. A repository default like `main` qualifies only when it is the checked-out branch. See [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Base derivation and merge target.
