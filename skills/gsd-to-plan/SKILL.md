...
Consume the converged `Domain Impact`. Record its fields in this exact order: `Classification`, `Contexts`, `Documentation`, `Broad bootstrap`, `Evidence`. `classification=none` requires `contexts=none`, `documentation=none`, and concrete no-impact evidence. Every other classification requires sorted context slugs and a documentation action. Bind the exact domain-documentation paths returned by `gsd-domain-modeling` to the same tasks as their implementing code. When the domain index exists, `Broad bootstrap` must be `not-offered`. When it is absent, record the user's independent `selected` or `declined` choice after mandatory feature-scoped bootstrap is established. Never reconstruct paths by scanning docs or dirty files.
