...
Consume the converged `Domain Impact`.
- Record its fields in this exact order:
- `Classification`, `Contexts`, `Documentation`, `Broad bootstrap`, `Evidence`.
- `classification=none` requires `contexts=none`, `documentation=none`, and concrete no-impact evidence.
- Every other classification requires sorted context slugs and a documentation action.
- Bind the exact reserved domain-documentation paths returned by `gsd-domain-modeling` to the same tasks as their implementing code; the plan owns target behavior until implementation, while existing domain prose remains current-production-only.
- When the domain index exists, `Broad bootstrap` must be `not-offered`.
- When it is absent, record the user's independent `selected` or `declined` choice after mandatory feature-scoped paths are established.
- Never reconstruct paths by scanning docs or dirty files.

## Write plan.md

Consume the converged `UI Impact` and write it directly after `Domain Impact`.
- Record its fields in this exact order: `Classification`, `Surfaces`, `Prototype`, `Evidence`.
- A locked `design/` prototype is the source of truth for the surface behavior it renders; the plan converts from it and never redefines it.
- `extend-prototype` and `new-prototype` author the prototype: bind each declared prototype path to a live task that also changes a non-doc `design/` artifact. `reuse-prototype` consumes an already locked prototype, so it names those paths as its source without owning an edit to them.
- Only `reuse-prototype` names production `Surfaces`; prototype-authoring work precedes any production surface and records `Surfaces: none`.
...
- Never use `none` for observable behavior; add the smallest real fast seam when needed.
