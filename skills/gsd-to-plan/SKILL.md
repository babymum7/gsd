...
- **Use only actual current spec AC IDs and cover them all.** Parse raw `spec.md` with the canonical active/superseded AC grammar in [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § spec.md — template & rules, including its read-only legacy all-bold compatibility for a complete pre-contract section. Malformed or mixed headers, missing/nonconcrete checks, duplicate IDs, or one ID in both states stop for spec repair rather than normalization. Every `satisfies` token on a non-superseded row must be a known, unique active AC ID; reject missing, duplicate, unknown (for example `AC-404`), or extra/conflicting pins. Cross-check the union across non-superseded rows against the active AC set before finishing — a missing AC is an incomplete plan, not a verify-time surprise. Superseded rows remain provenance only and may retain their obsolete AC references; never count or validate them as current coverage.
