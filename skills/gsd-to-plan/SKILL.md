...
Read § Base from the work tree, never from convention: record the branch from `git symbolic-ref --quiet --short HEAD` before `wip/<feature>` exists, so a linked worktree records its own branch. A detached HEAD stops packet creation until the user checks out a branch, because a commit oid can hold no squash; never substitute `git rev-parse --abbrev-ref HEAD`, which prints the literal `HEAD`. See [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Base derivation and merge target.
