...
1. Begin whole-diff review after all tasks and fast checks are green and before the complete feature-affected slow/E2E suite. There is no terminal pre-E2E visual pause. The parent dispatches the persistent gsd-reviewer agent (reusing the same gsd-reviewer session with the bound reviewer model from `modelRoles.gsdReviewer`). The reviewer does not require prior slow-suite evidence. If the independent reviewer capability or model configuration is unavailable, GSD must fail closed immediately.
2. Parent-mediated repair loop: the read-only reviewer returns structured findings to the parent; the parent alone updates state and directs the persistent gsd-executor agent to perform source-first repair with deterministic local fast checks (including fast acceptance/contract checks). Repeat whole-diff re-review until reviewer PASS on commit C, using the progress guard.
3. Run the complete feature-affected slow suite (Deferred Slow E2E) only after reviewer PASS on commit C. Reviewer and E2E never run concurrently.
4. E2E-failure repair: the parent forwards E2E failure evidence to the executor; the executor repairs source and reruns affected fast checks. Any changed bytes invalidate prior review and require whole-diff re-review with reviewer PASS again before the full slow/E2E suite reruns. Do not run reviewer and E2E in parallel.
5. Red gates/findings: source-first repair, smallest affected fast/slow subset, whole-diff re-review, then complete feature-affected slow/E2E only after reviewer PASS; repeat with progress guard. Terminal completion requires both the complete feature-affected slow suite and `gsdReviewer` whole-diff verdict to be green on final bytes. Merge requires reviewer PASS and complete slow/E2E GREEN on the same unchanged commit.
