...
2. Prove `Domain Impact` against cumulative diff: `none` requires concrete evidence that no term, invariant, workflow, outcome, relationship, policy, or bounded-context meaning changed. Non-`none` classifications require exact affected shards and index/AGENTS upserts owned by the same tasks as code; with existing index, broad-bootstrap offers/selections are contradictory.
...
- Non-`none` classifications require exactly one semantic-code task owning each affected shard; `Broad bootstrap` must always be `not-offered`.
