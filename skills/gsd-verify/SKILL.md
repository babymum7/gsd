...
4. After current-commit conformance, offer Terminal Visual Review when eligible. UI/UX plans always receive `Continue to Deferred Slow E2E` and `Visualize completed work with Lavish`; eligible substantial non-UI work receives the same surface. Ineligible work proceeds without a prompt. Continue does not launch a helper: Visualize invokes the internal Bun CLI directly against the completed live app or HTML target. Unavailable Lavish degrades to equivalent terminal inspection.
5. Terminal Visual Review is capture-only: browser feedback records evidence but never edits tracked source, begins repair, repeats conformance, or starts Deferred Slow E2E. Keep the main session interactive. Open the completed live app directly with `bun "$GSD_ROOT/tools/lavish/src/cli.ts" app "$URL"`, then immediately checkpoint the returned `SESSION_ID`, initial cursors, and current verified commit in opaque `next_action`. Before reporting that feedback is monitored, attach `bun "$GSD_ROOT/tools/lavish/src/cli.ts" poll "$SESSION_ID" --after 0 --after-reply 0` as a completion-aware foreground wait or harness-native tracked background job.
6. Queue operations remain private. When Send now completes the attached poll, summarize the ordered batch and attachment metadata, checkpoint `await terminal repair confirmation`, and present only `Start fixing`/`Continue feedback`; no acceptance and no repair before `Start fixing`. `Continue feedback` publishes the agent response and reattaches with `bun "$GSD_ROOT/tools/lavish/src/cli.ts" poll "$SESSION_ID" --after "$CURSOR" --after-reply "$REPLY_CURSOR" --agent-reply "$AGENT_REPLY"` without source mutation. Never claim monitoring between completion and re-entry. `feedback "$SESSION_ID"` is audit history only.
7. After `Start fixing`, bind the pending cursor and digest, end the session explicitly, and the session owner repairs only that frozen confirmed in-scope feedback set. Reject feedback that changes scope, acceptance, interface, invariant, or design as Spec escalation; rerun affected fast checks, conformance, and visual evidence. With zero pending feedback and current-commit conformance, present only `Accept visual result`/`Continue feedback`; no `Start fixing`. Explicit acceptance binds current-commit visual acceptance and ends the session explicitly. New feedback clears acceptance; source changes clear both conformance and visual acceptance.
...
After green merge, atomically write `phase=merged-cleanup-pending`; end any Lavish session and remove only its exact `.lavish/sessions/<session-id>/` directory after inspecting each entry with `lstat`. Never follow symlinks, remove another session, or touch the persistent project browser profile.
