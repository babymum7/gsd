...
1. If `manual_ui_review,on`, run one Manual UI Review Gate at terminal pre-E2E; show the exact run command or URL and visual checklist; await confirmation/feedback. Never per task or automated replacement.
2. In-scope: source-first persistent-executor repair, smallest affected Fast TDD subset, repeat gate. Scope/AC/interface/invariant changes escalate to `Discussion/Spec-escalation`; source change invalidates later fast/manual/slow evidence.
3. After confirmation/off, run the complete feature-affected slow suite (Deferred Slow E2E) only after all tasks and fast checks pass.
4. Begin whole-diff review only after the complete feature-affected slow suite is green. The parent dispatches the persistent gsd-reviewer agent (reusing the same gsd-reviewer session with the bound reviewer model from `modelRoles.gsdReviewer`). If the independent reviewer capability or model configuration is unavailable, GSD must fail closed immediately.
5. Red gates/findings: source-first repair, smallest affected fast/slow subset, complete slow suite, whole-diff re-review; repeat with progress guard. Terminal completion requires both the complete feature-affected slow suite and `gsdReviewer` whole-diff verdict to be green on final bytes.
