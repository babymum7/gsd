...
5. Terminal Visual Review is capture-only. Direct main-session instructions remain available; source changes clear both conformance and acceptance. Checkpoint `capture in progress` with sequence, launch commit, and artifact digest before invoking each destructive `lavish-axi poll` or fallback drain. Queue async results, reconcile revisions, record/read back the ledger, clear the marker before any replacement poll, and acknowledge feedback as recorded but not applied. Capture never edits tracked source, begins repair, runs Fast TDD, repeats conformance, or starts Deferred Slow E2E.
