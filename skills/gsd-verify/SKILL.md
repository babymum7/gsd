...
5. Terminal Visual Review is capture-only. Checkpoint `capture in progress` with the intended sequence before invoking each destructive `lavish-axi poll`. Record/read back the ledger under [REFERENCE.md § Post-approval pipeline contract](../gsd/REFERENCE.md#post-approval-pipeline-contract), clear the marker, and acknowledge feedback as recorded but not applied. Capture never edits tracked source, begins repair, runs Fast TDD, repeats conformance, or starts Deferred Slow E2E.
