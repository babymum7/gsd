...
The merge target is exactly the recorded `state.toon` `base_ref`; never ask whether to merge into `main` and never widen to the repository default. Before the squash run `node "<GSD_ROOT>/tools/gsd-git.mjs" preflight --feature-dir .scratch/<feature>`: only `status: ready` proceeds, and any `status: blocked` code is Spec escalation that stops the gate instead of retargeting the merge. Promoting that base onward is separate user-owned work after this packet ends green.
