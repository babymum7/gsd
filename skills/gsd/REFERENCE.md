...
- `enter terminal verification/repair`: requires `gsd-verify` and `gsd-handoff`. The same opaque `next_action` resumes cumulative whole-diff review/repair, optional Terminal Visual Review (including `gsd-lavish` when selected), and Deferred Slow E2E without new `state.toon` fields.
