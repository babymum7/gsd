...
The core pipeline is `gsd-brainstorming` → `gsd-to-plan` (writes the plan) → approval → `gsd-executing-plans` → `gsd-verify` → squash merge and result cleanup. After every complete draft plan, one post-plan action surface offers approve and execute, Build prototype with Lavish, revise, and pause/save. Execution stays automatic through Fast TDD, terminal whole-diff `gsdReviewer`, and Deferred Slow E2E; optional Terminal Visual Review may run after reviewer PASS when offered/selected. Scratch auto-deletes after a green merge unless retain or archive-and-delete was explicitly selected before final review; archive materialization is reference-only and never reopens planning or any other menu. A load-bearing spec gap returns to `gsd-brainstorming`, then a revised plan and approval; it never patches stale authority in place.
