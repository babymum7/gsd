...
8. **Lifecycle authority stays session-owner inline.** GSD dispatches no child repair, diagnosis, architecture, or verification task; implementation is dispatched only as parallel waves of provably independent tasks to sub-agents under [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Parallel wave dispatch, and the session owner remains sole lifecycle authority. A pre-existing delegation follows only its assignment.
