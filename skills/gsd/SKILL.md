...
**Step 0 — Apply the result-marker decision matrix first.** Before ordinary scratch plan.md routing, glob every `.scratch/<feature>/result.toon`, strictly validate all discovered records under [REFERENCE.md](REFERENCE.md) § Squash and cleanup result marker contract, then apply the ordered decision matrix there. A stopping decision performs only its named fail-closed, cleanup-question, cleanup-only, or block-resume action and emits no numbered route. A route-bearing decision continues through ordinary state detection with the matrix's relevance exclusions intact. Retained and residual markers never participate in modification-time selection; generic `continue` selects only live state.

**Step 0 — Canonical contract check.** Before ordinary routing, glob `.scratch/*/` for Markdown and runtime artifacts. A normal canonical `# Plan` without valid approval binding remains planning (`gsd-to-plan`); an unbound `# Quick-fix Plan` remains under the Quick-fix `gsd-verify` gate. An approved/executing feature additionally has a valid `plan.md` and an approval/runtime handoff (`handoff-*.toon`). The master does not perform a pre-route live plan hash/binding comparison; the selected executor/handoff owner skill performs the entry full parse/binding check and Spec escalation if a mismatch exists. Any legacy `proposal.md`, `spec.md`, or `design.md` is rejected. Any root `proposal.toon`, `spec.toon`, `design.toon`, or `plan.toon` is stale non-authoritative state: never read it for scope, task order, or acceptance; stop with actionable re-convergence.

**Step 0 — Detect state first (before matching routes).** Glob `.scratch/*/` for `plan.md` / `handoff-*.toon`, and scan the prompt for a pasted diff/PR. Workspace state — not just the prompt's wording — drives Routes 1/2/3: a "continue"/"resume" prompt with a live `handoff-*.toon` is Route 1 even when it reads like new work; a feature ask **related to** an existing feature with a `plan.md` is Route 3; an **unrelated** feature ask with a `plan.md` is Route 6 (new work), not Route 3. Resume-style prompt but no local `.scratch/` (fresh clone / other machine)? `git fetch --prune`, then list local + remote WIP branches: `git branch -a --list 'wip/*' --list '*/wip/*'` — a portable handoff materializes `.scratch/<feature>/` via `git switch --track origin/wip/<feature>` (or plain `git switch wip/<feature>` if local; see gsd-handoff § Portable); no synced scratch → reconstruct per gsd-handoff.
...
1. **Resume**:
   - If `.scratch/<feature>/handoff-<n>.toon` exists or is passed, delegated entry has been expressed via the Compaction Recovery Capsule (validated and rendered per the generic renderer protocol defined in [REFERENCE.md](REFERENCE.md)). Dispatch this delegated entry directly to ordinary gsd-handoff parsing, validation, and next_action execution. Do not invoke or execute the capsule again, avoiding circular re-entry and duplicate actions.
   - If no usable local handoff or plan.md can satisfy the continue intent (when `.scratch/` is absent or lacks these files) and the user intent is explicitly to continue/resume:
     - Scan tracked base-branch canonical Milestone Ledger paths (`docs/gsd/<feature>/milestones.md`) only after scratch/handoff/plan.md recovery cannot satisfy the continue intent.
...
3. **Spec/Plan**:
   - If `plan.md` exists but has no valid approval binding / no handoff → if it is a normal canonical `# Plan`, route to `gsd-to-plan` (in `Resume/finalize` mode); if it is an unbound `# Quick-fix Plan`, it remains under the Quick-fix `gsd-verify` gate.
   - If `plan.md` plus an approval/runtime handoff exist and the prompt relates to that feature's unfinished runtime work → route to `gsd-executing-plans`; that skill selects the Invocation Mode and validates the binding, so drift reaches its Spec-escalation path instead of changing the route. Select `Milestone plan execution` only from explicit milestone intent/entry context, then require active plan/scratch/WIP slug, canonical root path, first-pending base row, and exact-once plan ownership to agree; otherwise Normal mode or fail closed when milestone mode was explicitly claimed. (An unrelated prompt falls through to Route 4/5/6 — an existing plan is not a claim on every prompt.)
   - Normal mode has no milestone-completion authority. It may publish an authorized convergence-time ledger only under [REFERENCE.md](REFERENCE.md) § Convergence Ledger publication contract: the approved `plan.md` `## Publication` value must be the exact active root ledger path (not `null`), and one non-superseded `plan.md` task must own that exact path once in Files. The source binding, pending-only append/create bytes, owner-task commit evidence, and ordinary review/build/acceptance gates still apply. Ledger presence, changed bytes, or context cannot infer authority.
...
1. Reuse the code, docs, task brief, plan.md, and relevant domain shards already read for the selected route. Do not add a repository-wide glossary or decision sweep.
...
- Pastes plan.md/diff → **stress-test**.
- Vague one-liner → **discovery**.
- Materially ambiguous → ONE disambiguating question (see Clarify principle above).
- Pure question/advisory/exploration (no code change intended) → **answer directly**; no plan.md.
## Body
...
- **Right-size the recommendation**: recommend the smallest approach that meets the ask (ponytail-ladder thinking); a small ask converges to a 2-4-AC plan. Never pad a plan with speculative scope — retries, telemetry, config, abstractions nobody asked for are added when asked, not by default.
...
When Discovery converges, route to `gsd-to-plan` (in `Initial converged creation` mode) without writing Markdown. This plan source is the only pre-approval authority. Every active AC has a concrete Outcome, Action, Expected, invariant/non-goal context, and one pinned public seam. Keep unresolved or future work as one concise Discussion note; never convert it into a vague AC or task.

For an intentional approved Milestone Ledger publication, set `## Publication` in `plan.md` to the exact canonical ledger path whose slug exactly equals Feature; otherwise use `null`. The marker proves only planned publication intent, never completion authority. `gsd-to-plan` assigns every certain pre-approval domain write returned by `gsd-domain-modeling` to exactly one task Files field. Quick fixes carry their minimal `plan.md` only; Nano stays entirely git-free and needs no feature packet.
## Triggers (supporting skills fire automatically; lavish must *ask first*, launches only on accept)
 - `gsd-lavish` — visual surface for substantial, standalone deliverables (finalized `plan.md`, comparison, verify report, audit). **You MUST proactively ask before launching, and launch only when the user accepts** (per [REFERENCE.md](REFERENCE.md) § Lavish opt-in gate taxonomy): when the deliverable is offer-eligible and the **Gate (both must hold)** clears — (1) a reviewable deliverable, not mid-conversation; AND (2) annotating it in a browser adds value — surface the option folded into the surface already shown (one numbered end-session menu choice, e.g. "Review the plan visually"; or a single inline "review this visually?" line) — never a second prompt, never a new question around plan approval. Launching the browser flow waits for the user to accept. Never auto-launch, never on inline Qs or per-task diffs. Silently skipping the offer on an eligible deliverable is the bug this fixes.
...
TOON remains runtime-only: immutable task attempts, handoffs, result markers, and runtime counters/state. It never authors, reinterprets, or durably owns human-approved goals, plans, milestone ledgers, glossary prose, or decisions; attempts carry only lossless ordered copies of approved decisions for runtime transport.
Durable project documentation is strict Markdown: canonical plan.md plus domain docs/ledgers, bounded-context domain knowledge under `docs/domain/`, and optional milestone ledgers under `docs/gsd/<feature>/milestones.md`.
