...
The core pipeline is `gsd-brainstorming` → `gsd-to-plan` (writes the plan) → approval → `gsd-executing-plans` → `gsd-verify` → squash merge and result cleanup. After implementation checks pass and before final terminal review/squash, an enabled optional Manual UI Review Gate runs once before Deferred Slow E2E and terminal whole-diff `gsdReviewer`; the terminal scratch disposition (delete, retain, or archive-and-delete) remains the one required terminal disposition; archive materialization is reference-only and never reopens planning or any other menu. A load-bearing spec gap returns to `gsd-brainstorming`, then a revised plan and approval; it never patches stale authority in place.
