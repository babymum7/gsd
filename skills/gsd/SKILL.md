...
The core pipeline is `gsd-brainstorming` → `gsd-to-plan` (writes the plan) → approval → `gsd-executing-plans` → `gsd-verify` → squash merge and result cleanup. After every complete draft plan, one post-plan surface offers approve and execute, Build prototype with Lavish, revise, and pause/save. Execution stays automatic through Fast TDD, terminal whole-diff `gsdReviewer`, and Deferred Slow E2E. Terminal reviewer modes are the context-safe single path or Adaptive Chunked Cumulative Review before Terminal Visual Review or Deferred Slow E2E; optional Terminal Visual Review runs after merge-authoritative reviewer PASS when selected. Shard/reducer PASS never authorizes Terminal Visual Review, Deferred Slow E2E, or merge. Scratch auto-deletes after a green merge unless retain or archive-and-delete was selected before final review; archive materialization is reference-only and never reopens planning or any other menu. A load-bearing spec gap returns to `gsd-brainstorming`, then a revised plan and approval; it never patches stale authority in place.
