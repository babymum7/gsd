...
4. **Choose exactly one primary process owner.** Load its listed `SKILL.md` first, never several.
   Explicit new or changed user-facing surface intent, a named screen, page, or UI, selects `gsd-prototyping` before requirement convergence. A generic feature or integration request naming no surface converges through `gsd-brainstorming` first; backend-only work never routes through a prototype. Design-first default order; different/unclear asks one question.
   Drift or sync intent between the `design/` prototype and the codebase selects `gsd-design-sync`, which audits and routes but writes neither side.
...
The core pipeline is `gsd-brainstorming` → `gsd-to-plan` → approval → `gsd-executing-plans` → `gsd-verify` → squash cleanup. The top-level session is sole lifecycle authority, working inline. Every complete draft offers approve/execute, revise, and pause/save. Execution runs Fast TDD, deterministic terminal conformance, then Deferred Slow E2E; source changes invalidate terminal evidence.
