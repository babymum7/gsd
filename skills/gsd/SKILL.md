...
**Step 0 — Apply the result-marker decision matrix first.** Before ordinary scratch proposal/spec/plan routing, glob every `.scratch/<feature>/result.toon`, strictly validate all discovered records under [REFERENCE.md](REFERENCE.md) § Squash and cleanup result marker contract, then apply the ordered decision matrix there. A stopping decision performs only its named fail-closed, cleanup-question, cleanup-only, or block-resume action and emits no numbered route. A route-bearing decision continues through ordinary state detection with the matrix's relevance exclusions intact. Retained and residual markers never participate in modification-time selection; generic `continue` selects only live state.
