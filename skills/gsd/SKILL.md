...
4. **Choose exactly one primary process owner.** Load its listed `SKILL.md` first, never several.
   New or changed user-facing surface work selects `gsd-prototyping` before requirement convergence, because the `design/` prototype is the source of truth production UI converts from. Backend-only work never routes through a prototype.
