...
produces: [spec.md, plan.toon, docs/gsd/<feature>/milestones.toon]
consumes: [handoff-<n>.toon, plan.toon, spec.md, docs/gsd/<feature>/milestones.toon, CONTEXT.md, CONTEXT-MAP.md, docs/context/<area>/CONTEXT.md, docs/adr/]
---
...
3. **Spec/Plan**:
   ...
   - The publication exception requires the raw approved root `spec.md` to contain exactly one `Convergence Ledger publication` marker defined by [REFERENCE.md](REFERENCE.md), whose path equals the active root feature and sole plan owner, plus later selection of the Planned WIP gate. The marker and approved plan are necessary but not sufficient, and neither an ordinary Planned WIP nor Quick-fix WIP may infer authority from changed ledger bytes, path presence, or a context boolean.
...
- **Large-feature precision gate.** A materially answerable precise question—not a topic label—may keep a current/future milestone eligible for further Discussion, but only a materially precise goal whose wording and order the user approved may become a ledger row. Writing any `spec.md` still requires at least one checkable AC with its `Check:` stated concretely. Anything with neither stays exactly one concise fog/future/out-of-scope note on an existing Discussion/spec surface and creates no ledger row, detailed speculative spec, or plan task. For a mixed candidate, persist only the precise approved goals, put only fully checked ACs in `spec.md`, and collapse all unchecked remainder into that one note. Revisit it only after new evidence makes the goal or AC + `Check:` precise; never weaken this guard because a ledger already exists.
