...
produces: [plan.md, docs/gsd/<feature>/milestones.toon, .scratch/<feature>/tasks/<Tn>/a<N>.toon]
...
|---|---|---|---|---|
| Normal plan execution | `proposal.md`; `spec.md`; `plan.md` | `design.md`; `handoff-<n>.toon`; authorized ledger | `plan.md`; JIT attempt; authorized ledger | Stop as Spec escalation; never synthesize source state or dispatch |
| Milestone plan execution | Normal required state; authoritative ledger | `design.md`; `handoff-<n>.toon` | `plan.md`; JIT attempt; milestone ledger lifecycle state | Missing source is Spec escalation; missing ledger evidence is a Blocker |

## Intake and immutable contract

Read the complete Markdown packet from `.scratch/<feature>/`, parse it under [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Canonical Markdown contract, and compare all live source bytes to the approved source-set SHA-256 binding. Repeat this check on every handoff/resume, before dispatch, after repair, and before terminal verification. Missing, invalid, altered, or additional source is a Spec escalation. Do not infer intent from legacy pre-approval TOON; it is stale and non-authoritative.
