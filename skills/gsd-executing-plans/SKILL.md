...
2. Fan out one task per sub-agent, so single-task waves are exactly one sub-agent. Each sub-agent receives the full validated slice for exactly one task (step 2 of per-task loop), plan path, and base/WIP identity rebuilt from `plan.md` — never invented. Before sending, the owner re-reads each dispatch prompt against that task's retained slice and rebuilds any prompt missing a slice fact.
