...
Only after every non-superseded task and its Fast TDD Checks are green, update `state.toon` for terminal entry with `next_action` set to `enter terminal verification/repair` and load `gsd-verify` for one cumulative whole-diff review. Intermediate green task commits remain checkpoints with `next_action` set to `start/continue task` and never independently dispatch `gsdReviewer` or load `gsd-verify`. After reviewer PASS, `gsd-verify` may offer opt-in Terminal Visual Review before Deferred Slow E2E. Execution stays automatic through Fast TDD, whole-diff review, and Deferred Slow E2E; optional Terminal Visual Review may run after reviewer PASS when offered/selected. Explicit retain or archive-and-delete may be recorded in `cleanup_preference` before final review; otherwise scratch auto-deletes after green merge. Scope/AC/interface/invariant changes escalate to `Discussion/Spec-escalation`.
