...
6. Commit only green task-owned changes on WIP. Use `bun "<GSD_ROOT>/tools/gsd-state.mjs" set --feature-dir .scratch/<feature> key=value…` to atomically update `state.toon` with `last_green_task`, `last_green_commit`, `next_action=start/continue task`, and an amended plan hash when the plan changed; `write-state --json-file` remains the fallback (never the `write` tool directly). Mark that task done in the todo list in the same step as its green checkpoint. Task `Tn+1` begins only from the committed green checkpoint of `Tn`. Source mutations never overlap task/repair or Deferred Slow E2E.
...
4. Reconcile the wave: before merging, inspect each returned task — its diff stays inside the slice's paths and intents, its focused check re-runs green, and the diff carries no evident defect; a failed inspection returns to bounded inline repair instead of merging. Then merge each task branch into `wip/<feature>` in strict plan order, commit one green checkpoint, then write `state.toon` through `gsd-state.mjs` CLI exactly as step 6 with `last_green_task` set to the wave's last task. Mark wave tasks done in the todo list in the same step.
