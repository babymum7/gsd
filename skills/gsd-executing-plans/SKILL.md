...
3. The current top-level session owner consumes the validated slice and dispatches the task within its wave under [§ Parallel wave dispatch](#parallel-wave-dispatch) by default; when dispatch is unavailable it implements or repairs the task inline. GSD dispatches no repair task, generic child task, or parallel lifecycle work outside validated waves.
...
Every wave is dispatched to sub-agents by default, a single-task wave included, so the owner orchestrates and verifies instead of authoring task code; it remains sole lifecycle authority and reconciles every result. The canonical contract is [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Parallel wave dispatch.
