...
6. Commit only green task-owned changes on WIP. Use `gsd-state.mjs write-state --json-file` (never the `write` tool directly) to atomically update `state.toon` with `last_green_task`, `last_green_commit`, `next_action=start/continue task`, and an amended plan hash; delete the temp file. Mark that task done in the todo list in the same step as its green checkpoint. Task `Tn+1` begins only from the committed green checkpoint of `Tn`. Source mutations never overlap task/repair or Deferred Slow E2E.
