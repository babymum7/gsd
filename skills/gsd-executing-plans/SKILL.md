...
Only after every non-superseded task and its Fast TDD Checks are green, update `state.toon` for terminal entry with `next_action` set to `enter terminal verification/repair` and load `gsd-verify` for context-safe terminal review (small-path single cumulative review or Adaptive Chunked Cumulative Review). Intermediate green task commits remain checkpoints with `next_action` set to `start/continue task` and never independently dispatch `gsdReviewer` or load `gsd-verify`. Only after merge-authoritative single/root-integrator reviewer PASS (`mode=single` or root `mode=integrator`/`authority=merge`), `gsd-verify` may offer Terminal Visual Review before Deferred Slow E2E. Shard or reducer PASS never authorizes Terminal Visual Review, Deferred Slow E2E, or merge. Execution stays automatic through Fast TDD, whole-diff review, and Deferred Slow E2E; optional Terminal Visual Review runs only after that PASS when selected. Record explicit retain or archive-and-delete in `cleanup_preference` before final review; else scratch auto-deletes after green merge. Scope/AC/interface/invariant changes escalate to Spec-escalation.

## Auto-pilot

Approval is the last normal planning prompt. During execution report factual progress and blockers only; do not open unrelated menus or confirmations. A post-approval prototype request is Spec escalation, not a runtime visual gate. A manual pause or automatic context-pressure writes `phase=paused` and preserves the exact interrupted executable `next_action` before returning/stopping. A hard blocker writes `next_action` set to `Discussion/Spec-escalation` and `phase` recording the pause/block transition (e.g. `phase=paused`). Both use atomic `state.toon` writes via `gsd-handoff`.
