...
5. For non-`none` UI Impact, the locked `design/` prototype is the source of truth for the surface behavior the task converts. Update the locked prototype documentation in the same task as the surface change; never let production markup drift ahead of the prototype it came from.
