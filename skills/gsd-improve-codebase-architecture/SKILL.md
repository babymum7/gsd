...
When a durable domain signal already appears, read `docs/domain/index.md` and only the minimum mapped domain files; missing docs are normal. Trigger `gsd-domain-modeling` only for certain durable terms/decisions; never write domain artifacts here. Before approval, material meaning/ownership/trade-off ambiguity follows domain modeling's one-focused-question-and-no-write rule. Inside approved execution, ask zero documentation questions: load-bearing AC/interface/invariant ambiguity returns to `gsd-executing-plans` as Spec escalation; otherwise skip the write. Architecture investigation and deepening stay inline in the current top-level session.


## 2. Present candidates — terminal default, Lavish offer when eligible
Present candidates in the terminal by default. Before plan approval, when the audit/comparison is offer-eligible and both Fire gate checks in [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Lavish opt-in gate taxonomy hold, surface the Lavish option in the same presentation. After explicit selection, materialize only the requested comparison HTML under `.scratch/<feature>/prototype/` and invoke `bun "$GSD_ROOT/tools/lavish/src/cli.ts" prototype "$HTML_FILE"`. Retain `SESSION_ID`, attach `poll "$SESSION_ID"` before reporting monitoring, and publish each response with `--agent-reply` while reattaching; end the session explicitly after review. Ordinary post-approval task progress has no architecture Lavish offer; `gsd-verify` owns Terminal Visual Review after current-commit conformance. Do not load a bridge skill.
