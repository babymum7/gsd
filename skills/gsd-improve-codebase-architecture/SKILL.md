...
Do NOT write manual HTML. Present candidates in the terminal by default. Before plan approval, when the audit/comparison is offer-eligible and both Fire gate checks in [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Lavish opt-in gate taxonomy hold, surface the Lavish option in the same presentation. Ordinary post-approval task progress has no architecture Lavish offer; `gsd-verify` owns Terminal Visual Review only after current-commit session-owner conformance. Offer does not launch: load `gsd-lavish` only after explicit selection.
