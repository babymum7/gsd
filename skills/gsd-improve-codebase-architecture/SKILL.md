...
## 2. Present candidates — terminal default, lavish opt-in
Do NOT write manual HTML files. Compile the candidates and present them in the terminal by default. Before plan approval, architecture audits/comparisons may offer lavish when both Fire gate checks in [../gsd/REFERENCE.md](../gsd/REFERENCE.md) § Lavish opt-in gate taxonomy hold and post-approval pipeline no-offer mode is not active. An offer is not launch consent: launch `gsd-lavish` only after explicit opt-in; otherwise keep the deliverable in terminal.
