...
export function parseAgentsDomainSection(content) {
  ...
  const count = (name) => lines.filter((line) => line.trim() === `## ${name}`).length;
  const sections = ["Domain documentation", "Decisions", "Design"];
  for (const name of sections) {
    const found = count(name);
    if (found === 0) {
      throw new Error(`AGENTS.md must contain a \`## ${name}\` section`);
    }
    if (found > 1) {
      throw new Error(`AGENTS.md must contain exactly one \`## ${name}\` section`);
    }
  }
  return { domain: 1, decisions: 1, design: 1 };
}
