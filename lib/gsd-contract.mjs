...
export function parseQuickFixPlan(files) {
  ...
  if (domainImpact.classification !== "none") {
    ...
    const isProse = path => path.startsWith("docs/") || path === "AGENTS.md" || path.endsWith("/AGENTS.md");
    const isTest = path => path.startsWith("test/") || path.startsWith("tests/") || /(^|\/)[^/]+\.(test|spec)\.[^/.]+$/.test(path);
    const carriesSemanticCode = task => task.files.some(path => !isProse(path) && !isTest(path));
...
const PLAN_FEATURE_RE = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
const PLAN_SHA256_RE = /^[a-f0-9]{64}$/;
...
export function readPlanFile(planPath, { cwd = process.cwd() } = {}) {
  ...
  try {
    ...
    fail(`plan file cannot be read: ${error.message}`);
  } finally {
