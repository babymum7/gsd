...
function validateTitle(content, title) {
  ...
  if (!content.startsWith(`# ${title}\n## `)) {
    fail(`# ${title} must be followed directly by the first ## section`, `place the first ## section directly after the # ${title} title`);
  }
}
...
function section(content, heading, requiredSection = true) {
  ...
  if (matches.length > 1) fail(`duplicate ${heading} section`, `remove duplicate ## ${heading} section`);
  if (matches.length === 0) {
    if (requiredSection) fail(`missing ${heading} section`, `add ## ${heading} section`);
    return null;
  }
...
function validateSections(content, allowed, optional = []) {
  ...
  if (headings.length !== expected.length || headings.some((heading, index) => heading !== expected[index])) {
    fail(`sections must be exactly ordered: ${expected.join(", ")}`, "reorder sections to match the canonical order");
  }
}
...
function orderedFields(block, labels) {
  ...
  if (entries.length !== labels.length || entries.some(([, label], index) => label !== labels[index])) {
    fail(`fields must be exactly ordered: ${labels.join(", ")}`, "reorder fields to match the canonical order");
  }
...
function parseCriteria(content) {
  ...
  if (lines.some(line => line.trim() === "")) {
    fail("Acceptance Criteria must not contain blank or whitespace lines", "remove blank or whitespace lines from Acceptance Criteria");
  }
  if (lines.length === 0 || lines.length % 5 !== 0) {
    fail("Acceptance Criteria must consist of sequential AC blocks with no stray content", "format criteria as 5-line blocks: ### AC-N header followed by State, Outcome, Action, Expected");
  }
  ...
  for (let i = 0; i < numBlocks; i++) {
    ...
    if (!m1) fail(`Acceptance criterion block ${i + 1} heading is invalid or malformed`, "format heading as ### AC-N: Title");
    ...
    if (!m2 || !m3 || !m4 || !m5) {
      fail("fields must be exactly ordered: State, Outcome, Action, Expected", "reorder fields to match State, Outcome, Action, Expected");
    }
    ...
    if (ordinal !== i + 1) fail("criterion IDs must be sequential", "number criterion IDs sequentially starting from AC-1");
    if (ids.has(id)) fail(`duplicate criterion ${id}`, "ensure each criterion ID is unique and sequential");
    ids.add(id);

    if (state !== "active" && state !== "superseded") fail(`${id} has invalid state`, "set State to active or superseded");
    if (VAGUE.test(outcome) || VAGUE.test(action) || VAGUE.test(expected)) {
      fail(`${id} outcome, action, and expected must be concrete`, "describe specific observable behaviors and steps");
    }
...
function parseIdentifierList(content, heading, prefix) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!match) {
      fail(`${heading} line ${i + 1} does not match the canonical format`, `format line as - **${prefix}-N:** description`);
    }
    const [, idNumStr, text] = match;
    const ordinal = Number(idNumStr);
    if (ordinal !== i + 1) {
      fail(`${heading} IDs must equal ${prefix}-1 through ${prefix}-N in order`, `number ${heading} items sequentially from ${prefix}-1 to ${prefix}-N`);
    }
    const id = `${prefix}-${ordinal}`;
    const textVal = requiredExact(text, `${id} text`);
    if (VAGUE.test(textVal)) {
      fail(`${id} text must not be vague`, "describe a concrete constraint or boundary");
    }
...
function parseTasks(content, criteria) {
  ...
  const parseIdentity = (block, blockIndex) => {
    ...
    if (!satisfiesMatch) fail("fields must begin with Satisfies", "start task fields with - **Satisfies:**");
    ...
    if (satisfies.length === 0 || satisfies.some((criterion) => !known.has(criterion))) {
      ...
      fail(`${id} has unknown criterion${unknownCriterion ? `: ${unknownCriterion}` : ""}`, "reference an existing active criterion from Acceptance Criteria");
    }
    return {
      id,
      ordinal,
  ...
  const parseTestAndStatus = (id, testLine, statusLine) => {
    ...
    if (!testMatch || !statusMatch) {
      fail("fields must end with Test, Status", "end task fields with - **Test:** followed by - **Status:**");
    }
    const test = parseBacktickedCommand(testMatch[1], `${id} Test`);
    const status = requiredExact(statusMatch[1], `${id} Status`);
    if (!VALID_TASK_STATUSES.has(status)) {
      fail(`${id} has invalid status`, "set Status to pending, in_progress, done, or superseded");
    }
    return { test, status };
  };
  ...
  for (let blockIndex = 0; blockIndex < starts.length; blockIndex++) {
    ...
    if (block[2] !== "- **Files:**") {
      fail("structured task fields must be exactly ordered: Satisfies, Files, Test, Status", "reorder fields to match the canonical order");
    }
    let cursor = 3;
    const fileIntents = [];
    ...
    while (cursor < block.length && !block[cursor].startsWith("- **Test:**")) {
      ...
      if (!entry) fail(`${id} Files entry must contain a backticked path, create|modify|delete operation, and intent`, "format files entries as:   - `path` — create|modify|delete: intent");
      ...
      if (VAGUE.test(intent)) fail(`${id} Files intent must not be vague`, `specify what changes in ${pathValue}`);
      if (filePaths.has(pathValue)) fail(`${id} Files contains duplicate path: ${pathValue}`, "list each path at most once per task");
...
function parseQuickFixTasks(content) {
  ...
  for (let blockIndex = 0; blockIndex < starts.length; blockIndex++) {
    ...
    if (block[1] !== "- **Files:**") {
      fail(`${id} fields must be exactly ordered: Files, Test`, "reorder fields to match Files, Test");
    }
    ...
    while (cursor < block.length && !block[cursor].startsWith("- **Test:**")) {
      const entry = block[cursor].match(/^  - `([^`]+)` — (create|modify|delete): (.+)$/);
      if (!entry) {
        fail(`${id} Files entry must contain a backticked path, create|modify|delete operation, and intent`, "format files entries as:   - `path` — create|modify|delete: intent");
      }
      const [, rawPath, operation, rawIntent] = entry;
      const pathValue = validateRepositoryPath(rawPath, `${id} Files`);
      const intent = requiredExact(rawIntent, `${id} Files intent`);
      if (VAGUE.test(intent)) fail(`${id} Files intent must not be vague`, `specify what changes in ${pathValue}`);
      if (ownedPaths.has(pathValue)) fail(`Quick-fix Tasks contain duplicate path: ${pathValue}`, "list each path at most once across tasks");
    ...
    if (!testMatch) fail(`${id} fields must be exactly ordered: Files, Test`, "reorder fields to match Files, Test");
    const test = parseBacktickedCommand(testMatch[1], `${id} Test`);
...
function parseDecisions(content) {
  ...
  const lines = value.split("\n");
  if (lines.some(line => line.trim() === "")) {
    fail("Decisions must not contain blank or whitespace lines", "remove blank or whitespace lines from the Decisions section");
  }
  if (lines.length === 0 || lines.length % 3 !== 0) {
    fail("Decisions must be None. or sequential D blocks with no stray content", "set Decisions to None. or sequential 3-line ### D-N blocks");
  }
  const decisions = [];
  const ids = new Set();
  ...
  for (let i = 0; i < numBlocks; i++) {
    ...
    if (!m1) fail(`Decision block ${i + 1} heading is invalid or malformed`, "format heading as ### D-N: Title");
    const [, id, ordinalStr, title] = m1;
    const ordinal = Number(ordinalStr);
    ...
    if (!m2 || !m3) {
      fail("fields must be exactly ordered: Decision, Rationale", "reorder fields to match Decision, Rationale");
    }
    ...
    if (VAGUE.test(decision) || VAGUE.test(rationale)) {
      fail(`${id} decision and rationale must be concrete`, "provide a clear decision and rationale");
    }
    if (ordinal !== i + 1) fail("decision IDs must be sequential", "number decision IDs sequentially starting from D-1");
    if (ids.has(id)) fail(`duplicate decision ${id}`, "ensure each decision ID is unique and sequential");
    ids.add(id);
...
export function readPlanFile(planPath, { cwd = process.cwd() } = {}) {
  ...
  try {
    ...
    if (!rootStat.isDirectory() || rootStat.dev !== expected.rootDev || rootStat.ino !== expected.rootIno) {
      fail('workspace root identity changed before open', "ensure workspace directory is stable during validation");
    }
    ...
    if (!scratchStat.isDirectory() || scratchStat.dev !== expected.scratchDev || scratchStat.ino !== expected.scratchIno) {
      fail('.scratch identity changed before open', "ensure .scratch directory is stable during validation");
    }
    ...
    if (!featureStat.isDirectory() || featureStat.dev !== expected.featureDev || featureStat.ino !== expected.featureIno) {
      fail('feature directory identity changed before open', "ensure feature directory is stable during validation");
    }
...
export function normalizePlanFile(planPath, { cwd = process.cwd(), write = false } = {}) {
  ...
  if (write && normalized.fixes.length > 0) {
    ...
    const tempFile = path.join(dir, `.plan.md.${process.pid}.${Date.now()}.tmp`);
    try {
      fs.writeFileSync(tempFile, normalized.content, "utf8");
      fs.renameSync(tempFile, source.path);
    } catch (error) {
      try {
