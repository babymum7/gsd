...
export function validateDesignMap(docsPath, { cwd = process.cwd() } = {}) {
  ...
  // parent fds via /proc/self/fd. Single try/finally closes all fds in reverse order.
  let rootFd, designFd, docsFd;
  try {
    // Pin workspace root: lstat → open → fstat identity.
    const rootLstat = fs.lstatSync(workspaceRoot);
    if (rootLstat.isSymbolicLink() || !rootLstat.isDirectory()) fail('workspace root must be a real directory');
    rootFd = fs.openSync(workspaceRoot, DIR_FLAGS);
    const rootStat = fs.fstatSync(rootFd);
    if (!rootStat.isDirectory() || rootStat.dev !== rootLstat.dev || rootStat.ino !== rootLstat.ino) {
    ...
    if (!designStat.isDirectory()) fail('design must be a directory');

    // Pin docs/ through design fd.
    docsFd = fs.openSync(`/proc/self/fd/${designFd}/docs`, DIR_FLAGS);
    const docsStat = fs.fstatSync(docsFd);
    if (!docsStat.isDirectory()) fail('design/docs must be a directory');
