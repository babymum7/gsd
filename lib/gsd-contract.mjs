...
// the same criterion, or run the same focused check cannot be dispatched to
// parallel sub-agents because their green evidence would race. `parallel` marks
// only the waves that fan out to two or more concurrent sub-agents; a
// single-task wave is still dispatched, just to one sub-agent.
export function analyzeParallelWaves(tasks) {
  ...
  for (const task of active) {
    if (current === null) {
      current = { tasks: [task.id], parallel: false };
      waves.push(current);
      continue;
    }
    const independent = current.tasks.every((id) => tasksIndependent(task, byId.get(id)));
    if (independent) {
      current.tasks.push(task.id);
      current.parallel = current.tasks.length >= 2;
    } else {
      current = { tasks: [task.id], parallel: false };
      waves.push(current);
    }
...
function tasksIndependent(a, b) {
  ...
  if (a.satisfies.some((criterion) => b.satisfies.includes(criterion))) return false;
