...
export function generateUnifiedDiff(filePath, originalContent, fixedContent, context = 3) {
  ...
  const a = originalContent.endsWith("\n") ? originalContent.slice(0, -1).split("\n") : originalContent.split("\n");
  const b = fixedContent.endsWith("\n") ? fixedContent.slice(0, -1).split("\n") : fixedContent.split("\n");
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && a[i - 1] === b[j - 1]) {
      ...
      ops.push({ type: "insert", text: b[j - 1], newLine: j });
      j -= 1;
    } else {
      ops.push({ type: "delete", text: a[i - 1], oldLine: i });
      i -= 1;
    }
  }
  ops.reverse();
...
export function normalizePlan(content) {
  if (typeof content !== "string") {
    fail("plan content must be a string");
  }
