...
/**
 * Atomically open `relativePath` under `rootFd` via /proc/self/fd.
 * Opens each directory component through the pinned parent fd with O_NOFOLLOW,
 * preventing parent-directory swap attacks. Returns the opened file fd.
 * Caller MUST close the returned fd AND rootFd.
 */
function openAnchored(rootFd, relativePath) {
  if (process.platform !== 'linux') {
    fail('fd-anchored traversal requires Linux /proc/self/fd');
  }
  const segments = relativePath.split('/').filter(Boolean);
  let dirFd = rootFd;
  const intermediateFds = [];
  try {
    for (let i = 0; i < segments.length - 1; i++) {
      const childFd = fs.openSync(`/proc/self/fd/${dirFd}/${segments[i]}`, DIR_FLAGS);
      const childStat = fs.fstatSync(childFd);
      if (!childStat.isDirectory()) {
        fs.closeSync(childFd);
        fail(`${segments[i]} must be a directory`);
      }
      intermediateFds.push(childFd);
      dirFd = childFd;
    }
    const fileFd = fs.openSync(`/proc/self/fd/${dirFd}/${segments[segments.length - 1]}`, fs.constants.O_RDONLY | (fs.constants.O_NOFOLLOW ?? 0));
    return fileFd;
  } catch (error) {
    // Close intermediate dir fds only; rootFd is caller-owned.
    for (const fd of intermediateFds) try { fs.closeSync(fd); } catch { /* best effort */ }
    throw error;
  }
}
...
function resolveDesignDocsLocation(docsPath, cwd) {
  ...
  // Capture expected identities during validation so the caller can compare
  // them against opened fds, closing the gap between validation and pinning.
  const expected = {};
  const designDirectory = path.join(workspace, "design");
  for (const [candidate, label, store] of [
    [workspace, "workspace root", expected],
    [designDirectory, "design", expected],
    [absolute, "design/docs", expected],
  ]) {
    let inspected;
    try {
      inspected = fs.lstatSync(candidate);
    ...
    if (store !== undefined) {
      if (candidate === workspace) { store.rootDev = inspected.dev; store.rootIno = inspected.ino; }
      else if (candidate === designDirectory) { store.designDev = inspected.dev; store.designIno = inspected.ino; }
      else { store.docsDev = inspected.dev; store.docsIno = inspected.ino; }
    }
  }
...
// Read a design document through a pinned directory fd (fd-anchored, no pathname race).
function readDesignDocumentFd(docsFd, name, label) {
  let fd;
  try {
    fd = fs.openSync(`/proc/self/fd/${docsFd}/${name}`, fs.constants.O_RDONLY | (fs.constants.O_NOFOLLOW ?? 0) | fs.constants.O_NONBLOCK);
    const opened = fs.fstatSync(fd);
    if (!opened.isFile()) fail(`${label} must be a regular file`);
    const capacity = Math.min(DESIGN_DOC_MAX_BYTES + 1, opened.size + 1);
    const buffer = Buffer.allocUnsafe(Math.max(1, capacity));
    let total = 0;
    while (total < buffer.length) {
    ...
    if (total > DESIGN_DOC_MAX_BYTES) fail(`${label} exceeds ${DESIGN_DOC_MAX_BYTES} bytes`);
    ...
    let content;
    try { content = FATAL_UTF8_DECODER.decode(buffer.subarray(0, total)); } catch { fail(`${label} must be valid UTF-8`); }
...
export function validateDesignMap(docsPath, { cwd = process.cwd() } = {}) {
  const { absolute: directory, expected } = resolveDesignDocsLocation(docsPath, cwd);

  // fd-anchored traversal: pin workspace root → design dir → docs dir, all through
  // parent fds via /proc/self/fd. Compare each opened fd against the identities
  // captured during resolution to detect whole-root swaps between validation and pinning.
  let rootFd, designFd, docsFd;
  try {
    // Pin workspace root: open → fstat identity vs resolver.
    ...
    // Pin design/ through root fd.
    designFd = fs.openSync(`/proc/self/fd/${rootFd}/design`, DIR_FLAGS);
    const designStat = fs.fstatSync(designFd);
    if (!designStat.isDirectory() || designStat.dev !== expected.designDev || designStat.ino !== expected.designIno) {
      fail('design identity changed before open');
    }

    // Pin docs/ through design fd.
    docsFd = fs.openSync(`/proc/self/fd/${designFd}/docs`, DIR_FLAGS);
    const docsStat = fs.fstatSync(docsFd);
    if (!docsStat.isDirectory() || docsStat.dev !== expected.docsDev || docsStat.ino !== expected.docsIno) {
      fail('design/docs identity changed before open');
    }

    // Enumerate through the pinned fd to avoid pathname-based readdir race.
    const entries = fs.readdirSync(`/proc/self/fd/${docsFd}`, { withFileTypes: true });
    const names = entries
    ...
    for (const name of names) {
      ...
      const content = readDesignDocumentFd(docsFd, name, label);
    ...
    if (docsFd !== undefined) try { fs.closeSync(docsFd); } catch { /* best effort */ }
    if (designFd !== undefined) try { fs.closeSync(designFd); } catch { /* best effort */ }
    if (rootFd !== undefined) try { fs.closeSync(rootFd); } catch { /* best effort */ }
  }
}
