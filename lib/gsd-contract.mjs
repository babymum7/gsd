...
function parseCriteria(content) {
  ...
  if (lines.length === 0 || lines.length % 6 !== 0) {
    fail("Acceptance Criteria must consist of sequential AC blocks with no stray content", "format criteria as 6-line blocks: ### AC-N header followed by State, Outcome, Action, Expected, Scenario", bodyLine(lines.length));
  }

  const criteria = [];
  const ids = new Set();
  const numBlocks = lines.length / 6;

  for (let i = 0; i < numBlocks; i++) {
    const l1 = lines[i * 6];
    const l2 = lines[i * 6 + 1];
    const l3 = lines[i * 6 + 2];
    const l4 = lines[i * 6 + 3];
    const l5 = lines[i * 6 + 4];
    const l6 = lines[i * 6 + 5];

    const m1 = l1.match(/^### (AC-([1-9]\d*)): (.+)$/);
    if (!m1) fail(`Acceptance criterion block ${i + 1} heading is invalid or malformed`, "format heading as ### AC-N: Title", bodyLine(i * 6));
    ...
    const m6 = l6?.match(/^- \*\*Scenario:\*\* GIVEN (.+) WHEN (.+) THEN (.+)$/);
    if (!m2 || !m3 || !m4 || !m5 || !m6) {
      const badOffset = [Boolean(m2), Boolean(m3), Boolean(m4), Boolean(m5), Boolean(m6)].indexOf(false);
      fail("fields must be exactly ordered: State, Outcome, Action, Expected, Scenario", "reorder fields to match State, Outcome, Action, Expected, Scenario", bodyLine(i * 6 + badOffset + 1));
    }
    ...
    if (ordinal !== i + 1) fail("criterion IDs must be sequential", "number criterion IDs sequentially starting from AC-1", bodyLine(i * 6));
    if (ids.has(id)) fail(`duplicate criterion ${id}`, "ensure each criterion ID is unique and sequential", bodyLine(i * 6));
    ids.add(id);

    if (state !== "active" && state !== "superseded") fail(`${id} has invalid state`, "set State to active or superseded", bodyLine(i * 6 + 1));
    if (state === "active" && (VAGUE.test(scenario) || SCENARIO_PLACEHOLDER.test(scenario))) {
      fail(`${id} Scenario must be concrete`, "write GIVEN, WHEN, and THEN clauses with concrete preconditions, operations, and observable results", bodyLine(i * 6 + 5));
    }
    if (VAGUE.test(outcome) || VAGUE.test(action) || VAGUE.test(expected)) {
      const vagueRow = VAGUE.test(outcome) ? 2 : VAGUE.test(action) ? 3 : 4;
      fail(`${id} outcome, action, and expected must be concrete`, "describe specific observable behaviors and steps", bodyLine(i * 6 + vagueRow));
    }
