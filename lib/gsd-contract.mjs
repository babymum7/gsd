...
function parseCriteria(content) {
  ...
  for (let i = 0; i < numBlocks; i++) {
    ...
    if (!m2 || !m3 || !m4 || !m5) {
      const badRow = [m2 && 1, m3 && 2, m4 && 3, m5 && 4].find((offset) => !offset) ?? 1;
      fail("fields must be exactly ordered: State, Outcome, Action, Expected", "reorder fields to match State, Outcome, Action, Expected", bodyLine(i * 5 + badRow - 1));
    }
    ...
    if (VAGUE.test(outcome) || VAGUE.test(action) || VAGUE.test(expected)) {
      ...
      fail(`${id} outcome, action, and expected must be concrete`, "describe specific observable behaviors and steps", bodyLine(i * 5 + vagueRow - 1));
    }
