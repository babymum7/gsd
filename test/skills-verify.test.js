...
test("AC-4: Cross-references, None. explicit, repair evidence not duplicated, and renderer serialization", () => {
  ...
  assert.match(reference, /In Normal mode, `<features>`[\s\S]{0,20}serialized as all names joined/i);
  assert.match(reference, /In Bounded-Ambiguity mode, `<features>`[\s\S]{0,40}first 5 sorted features[\s\S]{0,60}omittedCount/i);
  ...
  assert.match(reference, /Bounded-Ambiguity mode[\s\S]{0,500}omitted from this list[\s\S]{0,10}stop and select exactly one active feature before resuming/i);
});
