...
2. Build the surface with any AI design tool configured against this repository: its working directory is set to the repository root, its meta directory is set to `design/` so the agent session it opens is contained there, its generated design files are targeted at `design/`, and the root `AGENTS.md` plus `design/DESIGN.md` are supplied as its context rather than derived from where the agent runs. The run's agent writes files; a run that returns a single inline artifact block is rejected. Every prototype artifact stays under `design/` and the tool's runtime output stays uncommitted. A single-file artifact is decomposed into separate files, components, and surface documents before lock.
