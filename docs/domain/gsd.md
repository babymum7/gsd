...
- Session Owner — owns discovery, planning, implementation, repair, verification, Git, merge, and cleanup inline, and dispatches implementation as validated waves of provably independent tasks to sub-agents by default, keeping inline implementation as the fallback.
...
- **Policy:** The current top-level session owns plan interpretation, implementation, repair, verification, E2E, Git, merge, and cleanup inline, and dispatches implementation as validated waves of provably independent tasks to sub-agents it then reconciles, which is the default even for a single task.
- **Reason:** One reconciling owner avoids lossy handoff even when every task is authored by a sub-agent, while canonical artifacts let a later session assume the role safely.
