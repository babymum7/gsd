...
- A discovered full malformed packet (`plan.md` beside a `state.toon` that is unparsable, a symlink, or another non-regular file) is handled differently depending on the caller. During **ordinary routing and explicit reads**, `detectCandidates` is called without fault tolerance and throws before any relatedness or terminal test runs, so a malformed packet blocks all candidate selection — including a prompt naming another valid feature. During **autocompaction** (`session.compacting`), `detectCandidates` is called with `{ faultTolerant: true }`: each malformed packet is caught and skipped individually (logged as a defect), so valid candidates survive alongside it. An all-malformed result produces no capsule and session_compact emits nothing. The same rejected bytes without a `plan.md` are residue in both contexts: discovery skips them and routes ordinarily, so the plan.md test decides the verdict rather than the order the directory entries happen to be read. Since unparsable bytes cannot be trusted, only the `.scratch/<feature>/` directory name is a relatedness signal.
