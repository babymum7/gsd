...
- Session Owner — owns discovery, planning, repair, verification, Git, merge, and cleanup inline, executes single-task waves inline with `gsd-tdd`, dispatches waves of two or more tasks to isolated per-task sub-agents via per-request isolation when the host enables task isolation (concurrently in separate isolated workspaces on separate task branches when global `task.isolation.enabled` is true, with `merge=branch`/`apply=false` for the owner-reconciled branch-gate form; serially in plan order when isolation is unavailable — including the stock-default `enabled=false`, where the measured per-request flag is not honored — or an isolated spawn fails), verifies each dispatch prompt against the slice before sending, reconciles returned tasks through the ordered five-layer gate before merging task branches into `wip/<feature>` in strict plan order, re-runs every merged task's focused check on that tree before writing the checkpoint, retires all wave-dispatched task branches and isolated workspaces before the squash while keeping unmerged work unforced, and implements inline when dispatch is unavailable.
...
- A fix already diagnosed stays ordinary direct work and never selects a visible owner: a prompt naming the file/line or exact failure signature is located work, so `gsd-diagnosing-bugs` owns only an unlocated or non-obvious cause. A bounded behavioral fix enters the Quick-fix lane through three size gates — Quick-fix grammar fit (one or two tasks), Domain Impact `none` or a single shard, and acceptance already converged from the prompt, with prior diagnosis not required — opened inline by the session owner with `gsd-tdd`, where `gsd-verify` gates only that existing packet; the owner proves the grammar fit deterministically by running `validate-quick-fix` on the draft plan before writing state. A returned Quick-fix WIP Fail leaves a repair round the prompt can name, and naming it re-enters that same `gsd-verify` gate instead of being answered directly. Ponytail remains hidden and never enters the matrix or runtime state.
...
- Implementation is the only lifecycle work GSD may dispatch, and only as validated waves of provably independent tasks to sub-agents: a wave is a maximal contiguous run of non-superseded tasks in strict heading order where every pair is file-, criterion-, and check-disjoint, computed deterministically by the Contract Validator's `analyze-waves`. Single-task waves execute inline by the session owner with `gsd-tdd`; waves of two or more tasks dispatch to isolated per-task sub-agents, so sub-agents author planned task code while the owner orchestrates, reconciles, and verifies; inline implementation by the owner is the fallback when dispatch is unavailable. A sub-agent receives one complete validated task slice, performs Fast TDD and its affected domain-doc updates in the same commit, works in its own isolated workspace on its own task branch cut from wave base, and mutates no lifecycle state. The owner inspects each returned task before merging in strict plan order, and repairs a failed task inline.
