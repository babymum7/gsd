...
| Deferred Slow E2E | A browser, GUI, external-service, long-lived-server, large-fixture, or otherwise resource-heavy feature journey run only after every implementation task and fast check is green and independent whole-diff review has reached reviewer PASS on the current commit. Failures enter parent-mediated executor repair with affected fast checks; any changed bytes require whole-diff re-review and reviewer PASS before the complete feature-affected slow suite reruns. | task focused check, per-task browser test |
...
- **Decision:** Every task with observable behavior must load `gsd-tdd` and use a Fast TDD Check for RED→GREEN→refactor. Planning adds the smallest real fast public seam when none exists. The executor runs only deterministic local unit, integration, CLI, or contract checks during implementation and repair; it never runs browser, resource-heavy, slow, or E2E suites. After all tasks and fast checks are green, run independent whole-diff review first. Run the complete feature-affected slow/E2E suite only after reviewer PASS on the current commit.
- **Rationale:** Fast feedback preserves test-first correctness while reviewer-first ordering avoids paying the highest machine and latency cost for a diff that independent review would reject. A final same-commit review and E2E invariant preserves correctness without redundant early full-suite runs.
...
- **Decision:** Do not dispatch `gsdReviewer` per task. After all tasks and fast checks pass, the read-only reviewer evaluates the whole WIP diff and returns structured findings to the parent; the parent sends blocking findings to the persistent executor, which repairs source and reruns affected fast checks. Only after reviewer PASS does the parent run the complete feature-affected slow/E2E suite. An E2E failure returns evidence to the executor; any repair that changes bytes invalidates the prior review, so the repaired commit must pass reviewer again before slow/E2E reruns. Merge requires reviewer PASS and complete slow/E2E GREEN on the same unchanged commit.
- **Rationale:** Separation keeps review independent and source mutation owned by the executor, while reviewer-first cadence avoids expensive E2E work on known-bad diffs. Same-commit invalidation prevents stale review or test evidence from authorizing merge.
