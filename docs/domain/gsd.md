...
- Session Owner — owns discovery, planning, repair, verification, Git, merge, and cleanup inline, dispatches every validated wave of provably independent implementation tasks to sub-agents — waves of two or more tasks concurrently, each task in its own isolated workspace on its own task branch; serially in plan order, the same validated slices one task at a time, when task isolation is unavailable or an isolated spawn fails — verifies each dispatch prompt against the slice before sending, inspects each returned task before merging its task branch into `wip/<feature>` in strict plan order, and implements inline only when dispatch is unavailable.
...
- Both Git facts the base rule depends on are observed by a read-only tool rather than asserted in prose: `tools/gsd-git.mjs derive-base` prints the branch to record at creation, and `tools/gsd-git.mjs preflight` proves before the squash that HEAD is attached, that `base_ref` and `wip_branch` still resolve to local branches, that the base is not checked out in another linked worktree, that the scratch plan bytes match the bound `plan_sha256`, and that nothing outside `.scratch/` is staged, modified, or untracked, because the commit recording a squash takes the whole index and would otherwise carry unreviewed bytes. Both paths of a staged rename count, since Git names only the destination first and a reviewed file moved into `.scratch/` would otherwise hide its own deletion. Its `status: blocked` codes stop the gate, and a Git query that cannot answer blocks as well, because an unanswered query proves nothing. Every Git call passes through one guard that admits only the exact read-only argument vectors it issues, never a subcommand name, because `git symbolic-ref <name> <ref>` writes the ref that `git symbolic-ref --quiet --short HEAD` merely reads; the tree query runs lock-free so that reading cannot even refresh the index, and `state.toon` is inspected without migration.
...
- Implementation is the only lifecycle work GSD may dispatch, and only as validated waves of provably independent tasks to sub-agents: a wave is a maximal contiguous run of non-superseded tasks in strict heading order where every pair is file-, criterion-, and check-disjoint, computed deterministically by the Contract Validator's `analyze-waves`. Every wave is dispatched, a single-task wave to exactly one sub-agent, so sub-agents author planned task code while the owner orchestrates, reconciles, and verifies; inline implementation by the owner is the fallback when dispatch is unavailable. A sub-agent receives one complete validated task slice, performs Fast TDD and its affected domain-doc updates in the same commit, works in its own isolated workspace on its own task branch cut from wave base, and never writes `state.toon`, amends `plan.md`, merges, or decides lifecycle. When task isolation is unavailable or an isolated spawn fails, the owner dispatches the wave serially in plan order, the same validated slices one task at a time, and never runs two tasks in one shared working tree; before merging, the owner inspects each returned task — diff scope against the slice, its focused check re-run green, the diff read for evident defects — and any failed inspection returns to bounded inline repair. The session owner reconciles the wave by merging each task branch into `wip/<feature>` in strict plan order into one green checkpoint, so `state.toon` remains the sole resumable authority and terminal conformance still proves the unchanged final commit.
...
- **Policy:** The current top-level session owns plan interpretation, repair, verification, E2E, Git, merge, and cleanup inline, verifies each dispatch prompt against the slice before sending it, dispatches concurrent wave tasks into separate isolated workspaces (serially in plan order, the same validated slices one task at a time, when isolation is unavailable or an isolated spawn fails), inspects each returned task — diff scope, focused check, and diff read — and then merges each task branch into `wip/<feature>` in strict plan order, keeping inline implementation as the fallback.
- **Reason:** Task authorship is not lifecycle authority: one reconciling owner avoids lossy handoff even when every task is authored by a sub-agent, while canonical artifacts let a later session assume the role safely.
