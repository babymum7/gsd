...
- Both Git facts the base rule depends on are observed by a read-only tool rather than asserted in prose: `tools/gsd-git.mjs derive-base` prints the branch to record at creation, and `tools/gsd-git.mjs preflight` proves before the squash that HEAD is attached, that `base_ref` and `wip_branch` still resolve to local branches, and that the base is not checked out in another linked worktree. Its `status: blocked` codes stop the gate, and a Git query that cannot answer blocks as well, because an unanswered query proves nothing. It funnels every Git call through one guard that admits only `rev-parse`, `symbolic-ref`, `show-ref`, and `worktree list`, so observing Git can never change it, and it inspects `state.toon` without migrating it.
