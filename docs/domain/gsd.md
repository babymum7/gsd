...
- Session Owner — owns discovery, planning, repair, verification, Git, merge, and cleanup inline, dispatches every validated wave of provably independent implementation tasks to sub-agents, and implements inline only when dispatch is unavailable.
...
- **Policy:** The current top-level session owns plan interpretation, repair, verification, E2E, Git, merge, and cleanup inline, and dispatches every validated wave of provably independent implementation tasks to sub-agents it then reconciles, keeping inline implementation as the fallback.
