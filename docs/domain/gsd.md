...
| Deferred Slow E2E | A browser, GUI, external-service, long-lived-server, large-fixture, or otherwise resource-heavy feature journey run only after current-commit deterministic conformance. | task-loop check, pre-verification suite |
...
- **Decision:** The current top-level session owns plan interpretation, implementation, repair, verification, E2E, Git, merge, and cleanup inline and sequentially.
...
- **Decision:** On resume, validate `schema:v3`, exact plan path/hash, base/WIP identity, last green task/commit, and current tree before rebuilding the active slice.
...
- **Decision:** Every observable task loads `gsd-tdd` and runs RED→GREEN→refactor at the smallest real fast public seam. Deferred Slow E2E runs only after current-commit terminal conformance.
...
### D-gsd-9: Collapse runtime history into one resumable snapshot
...
### D-gsd-10: Clean transient feature artifacts after green merge

- **Decision:** Delete feature scratch automatically after a green merge unless retain or archive-and-delete was selected.
