...
- Both Git facts the base rule depends on are observed by a read-only tool rather than asserted in prose: `tools/gsd-git.mjs derive-base` prints the branch to record at creation, and `tools/gsd-git.mjs preflight` proves before the squash that HEAD is attached, that `base_ref` and `wip_branch` still resolve to local branches, that the base is not checked out in another linked worktree, and that nothing outside `.scratch/` is staged, modified, or untracked, because the commit recording a squash takes the whole index and would otherwise carry unreviewed bytes. Its `status: blocked` codes stop the gate, and a Git query that cannot answer blocks as well, because an unanswered query proves nothing. Every Git call passes through one guard that admits only the exact read-only argument vectors it issues, never a subcommand name, because `git symbolic-ref <name> <ref>` writes the ref that `git symbolic-ref --quiet --short HEAD` merely reads; the tree query runs lock-free so that reading cannot even refresh the index, and `state.toon` is inspected without migration.
