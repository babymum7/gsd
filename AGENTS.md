...
- Any AI design tool may write into `design/`; ignore its runtime output rather than committing it. A tool that emits one single-file HTML dump has produced an input, not a resting state: decompose it into that separated structure before the surface locks. Never leave a locked surface as one undifferentiated file.
