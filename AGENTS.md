...
- A locked prototype is the source of truth for user-facing surface behavior, and design-first is the default order: change `design/` first, lock it, then convert the production UI from it. That order is a default, not a gate. Design work may be opened from the repository root as normal work, an already-locked surface may be changed again, and when the user describes a different order or the order is unclear, ask one question and take the answer as given rather than refusing or assuming a guard.
...
- Each surface document declares the production paths that surface governs in its own machine-read `## Production surfaces` section, holding either the sorted claim lines or exactly `none` before conversion. One production path is claimed by at most one surface document, so `node "<GSD_ROOT>/tools/gsd-contract.mjs" validate-design-map --path design/docs` can attribute drift to exactly one surface.
- Each surface document also declares its machine-read `## Conversion` section, whose body is exactly one token: `converted` or `pending`. A locked surface may stay `pending` while its conversion waits for a later batch, so a deferred conversion is a declared queue entry rather than a remembered intention, and `converted` requires at least one claimed production path. A change that converts a locked surface into production code flips that token to `converted` in the same task as the production change, so the declaration never outlives the code that satisfied it.
- When production has already moved ahead of its locked surface, back-port the change into `design/` and re-lock it before converting anything further; never treat the drifted production markup as the surface's new truth without recording it in the prototype it came from.
