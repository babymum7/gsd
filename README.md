...
The two committed reports on the current bytes each score three models in one run, at fingerprint `b938c4806936`. With the canon loaded, `a6/deepseek-v4.1-flash` scored 43/44 first attempt, `a6/gemini-3.8-flash` 44/44, and `a6/glm-5.3-flash` 42/44, for 129/132 (97.7%) overall; each reached 44/44 after the correction pass. The bootstrap-only lower bound scored 40/44, 40/44, and 39/44 for 119/132 (90.2%), and its misses concentrate on terminal-state rows (`result-*`) — exactly the rows the on-demand canon owns, which is why the canon scope is the operative measure and the bootstrap stays lean with a `§` pointer instead of that matrix.
