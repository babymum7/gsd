...
4. **Verification.** `gsd-verify` dispatches `gsdReviewer` for whole-diff terminal review after all tasks and fast checks pass and before Deferred Slow E2E; parent-mediated executor repair loops until reviewer PASS on commit C, then runs the complete feature-affected slow suite only after reviewer PASS; E2E failures return via the parent for fast repair and re-review of changed bytes before full E2E reruns. Reviewer and E2E never run concurrently. Merge requires reviewer PASS and complete slow/E2E GREEN on the same unchanged commit. Repair is source-first with smallest-affected and progress-guarded re-review/slow-suite loops. A red gate blocks merging. Scratch auto-deletes after a green merge unless retain or archive-and-delete was explicitly selected before final review.
