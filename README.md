...
The installer publishes a direct extension symlink at `~/.omp/agent/extensions/gsd-context.js`, or — when `PI_CODING_AGENT_DIR` is set to an absolute path — into that directory, because omp relocates its whole agent dir (config, commands, agents, and extension discovery) there and would otherwise never read a home-dir publish. A relative `PI_CODING_AGENT_DIR` fails closed before any publication; omp resolves it against each session's working directory. There is no wrapper; target collisions fail closed before publication. It installs no persistent model agent or model-role configuration. Upgrade preflight removes only positively recognized managed legacy GSD agent links; regular files, directories, foreign links, live unrelated links, and unrelated agents fail closed or remain unchanged. It does not install an OMP command or copy/link skills into a user skill directory. After publishing, it reports the machine's global OMP `task.isolation` settings against decision 0004 (`enabled: true`, `merge: branch`, `apply: false`) and, for each opposing value, repairs it via `omp config set` — a machine-global change affecting every OMP session, with revert commands printed — or prints the exact remediation commands when `omp` is absent. The check is advisory: it never fails the install.
