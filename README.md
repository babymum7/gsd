...
The two committed reports on the current bytes each score two models in one run, at fingerprint `418f3741e077`. With the canon loaded, `a6/deepseek-v4.1-flash` scored 39/40 first attempt and `a6/gemini-3.8-flash` 38/40, for 77/80 (96.3%) overall. The bootstrap-only lower bound scored 35/40 and 35/40 for 70/80 (87.5%), and its misses concentrate on the terminal-state rows (`result-*`) and the compaction rows — exactly the rows the on-demand canon owns, which is why the canon scope is the operative measure and the bootstrap stays lean with a `§` pointer instead of that matrix.
...
Skill compliance is the narrow third axis: it proves that a model actually performs the bootstrap-required first action, not merely names the skill. `GSD_EVAL_MODEL=a6/deepseek-v4.1-flash,a6/gemini-3.8-flash bun test/eval/skill-compliance-eval.mjs` runs the 25 activation fixtures that expect a primary skill in isolated OMP JSON sessions, enables only the `read` tool, and passes a fixture only when the first visible assistant action is an exact `read` of that skill's catalog path. The committed report currently scores those models 22/25 and 24/25, for 46/50 aggregate, and records the concrete first-action miss for each failure; use `--report-path` to write a fresh report without touching the committed one.
