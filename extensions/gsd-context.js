...
Key routing rules: ordinary-routing and ignore-terminal-record use load or direct; cleanup-question, cleanup-only, block-resume, and fail-closed use stop. Plan-hash mismatch does not override the normal owner: bare continue still enters gsd-handoff; prompt-named pending execution work enters gsd-executing-plans. Explicit diff or PR review always loads gsd-verify, never direct. plan.md beside malformed state.toon fail-closes before any direct/nano routing. Quick-fix enters by three size gates (one or two tasks, none or single-shard Domain Impact, converged acceptance) without prior diagnosis; single-task waves execute inline with gsd-tdd, waves of two or more tasks dispatch isolated sub-agents.
